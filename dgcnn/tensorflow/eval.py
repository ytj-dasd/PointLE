...
parser.add_argument('--model_path', default='log/10model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
...
TEST_FILES = provider.getDataFiles(\
    os.path.join(BASE_DIR, 'data/modelnet40_ply_hdf5_2048/val.txt'))
...
def eval_one_epoch(num_votes=1):
    error_cnt = 0
    is_training = False
    total_correct = 0
    total_seen = 0
    ...
    for fn in range(len(TEST_FILES)):
        with tf.Graph().as_default():
            ...
            for batch_idx in range(num_batches):
                ...
                for vote_idx in range(num_votes):
                    ...
                    #print(name)
                    output = sess.run(tf.get_default_graph().get_operation_by_name('fc2/bn/batchnorm/add_1').outputs[0], feed_dict=feed_dict)
                    for k in range(255):
        ...
        tf.reset_default_graph()
            ...
            #pred_val = np.argmax(object_score)
            #pred_sum = np.max(object_score)
            #print(int(pred_val),float(pred_sum))
            #print(7,float(object_score[0,7])) 
