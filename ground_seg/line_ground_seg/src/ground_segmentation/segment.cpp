...
namespace line{
Segment::Segment(const unsigned int& nbins_,
...
void Segment::fitSegmentLines() {
  ...
  while (!line_start->hasPoint() || line_start->getMinZPoint().z - cur_ground_height > max_start_height_) {
    ++line_start;
  ...
  // std::cout << "  points: " << has_point_num << std::endl;
}
...
double Segment::verticalDistanceToLine(const double &d, const double &z) {
  static const double kMargin = 0.8;
  double distance = -1;
...
bool Segment::findLineBegin(double cur_ground_height, std::vector<Bin>::iterator it) {
    ...
    while (std::fabs(it->getMinZPoint().z - cur_ground_height) > max_start_height_) {
        ++it;
        // Stop if we reached last point.
        if (it == bins_.end()) {
            has_finded = false;
...
void Segment::zsFitSegmentLines() {

    double cur_ground_height = -sensor_height_;
    bool is_long_line = false;
    std::list<Bin::MinZPoint> current_line_points;
    LocalLine cur_line = std::make_pair(0,0);
    for (auto line_iter = bins_.begin(); line_iter != bins_.end(); ++line_iter) {
        if (!line_iter->hasPoint()) {
            continue;
        }
        if (current_line_points.size() == 0){
            ...
            current_line_points.push_back(line_iter->getMinZPoint());
        ...
        }else if (current_line_points.size() >= 2) {
            Bin::MinZPoint cur_point = line_iter->getMinZPoint();
            is_long_line = 
                cur_point.d - current_line_points.back().d > long_threshold_;
            // Get expected z value to possibly reject far away points.
            double expected_z = cur_point.z;
            if (is_long_line && current_line_points.size() > 2) {
                expected_z = cur_line.first * cur_point.d + cur_line.second;
            }
            current_line_points.push_back(cur_point);
