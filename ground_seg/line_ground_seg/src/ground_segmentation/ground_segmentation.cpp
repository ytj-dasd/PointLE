...
//     params_ = params;
//     segments_ = std::vector<Segment>(params.n_segments, 
//                                     Segment(params.n_bins,
//                                             params.max_slope,
//                                             params.max_error_square,
//                                             params.long_threshold,
//                                             params.max_long_height,
...
void GroundSegmentation::segment(const PointCloud& cloud, std::vector<int>* segmentation) {
  // std::cout << "Line segmenting cloud with " << cloud.size() << " points. ";
  ...
  // saveSegmentPoint("/home/zs/zs/kitti/data/ground_seg", segmentation);
...
void GroundSegmentation::lineFitThread(const unsigned int start_index,
  ...
  for (unsigned int i = start_index; i < end_index; ++i) {
    ...
    if (visualize) {
      ...
      // lines_mutex->lock();
      // const pcl::PointXYZ line_start(0, 0, 0);
      // const pcl::PointXYZ line_end(cos(angle) * sqrt(params_.r_max_square),
      //                             sin(angle) * sqrt(params_.r_max_square), 0);
...
void GroundSegmentation::saveSegmentPoint(std::string file_path, const std::vector<int>* segmentation) {
    ...
    for (int i = 0; i < segmentation->size(); ++i) {
        ...
        std::ofstream file;
        file.open(file_path + "/" + std::to_string(bin_index_[i].first) + ".txt", 
        ...
        file << segment_coordinates_[i].d << " "
             << segment_coordinates_[i].z << "\n";
        file.close();
    }
}
