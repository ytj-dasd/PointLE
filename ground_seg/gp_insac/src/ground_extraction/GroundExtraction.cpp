...
GroundExtraction::GroundExtraction(ros::NodeHandle & node, 
    ...
    m_oGroundPub = node.advertise<sensor_msgs::PointCloud2>("/ground/pointcloud", 2);
    //publish topic of boundary clouds 
    m_oBoundPub = node.advertise<sensor_msgs::PointCloud2>("/boundary/pointcloud", 2);
    //publish topic of point clouds 
    m_oObstaclePub  = node.advertise<sensor_msgs::PointCloud2>("/obstacle/pointcloud", 2);
}
...
void GroundExtraction::HandlePointClouds(const sensor_msgs::PointCloud2 & vLaserData)
{
    ...
    groundExtract(vInputCloud, vBoundCloud, vGroundCloud, vBoundCloud);
...
void GroundExtraction::groundExtract(const pcl::PointCloud<pcl::PointXYZ>& cloud_in,
                                     pcl::PointCloud<PointXYZLO>& cloud_label){
    pcl::PointCloud<pcl::PointXYZ> process_Cloud;
    pcl::copyPointCloud(cloud_in, process_Cloud);
    ...
    std::vector<std::vector<GroundFeature> > vGroundFeatures = 
        oSectorDivider.ComputePointSectorIdxs(process_Cloud, oPointSecIdxs);
    ...
    //assigment of result in whole point clouds
    cloud_label.points.resize(process_Cloud.points.size());
    cloud_label.header = process_Cloud.header;
    cloud_label.width = static_cast<uint32_t> (process_Cloud.size ());
    cloud_label.height = 1;
    cloud_label.is_dense = process_Cloud.is_dense;
    cloud_label.sensor_orientation_ = process_Cloud.sensor_orientation_;
    cloud_label.sensor_origin_ = process_Cloud.sensor_origin_;
    //assignment in whole point clouds
    for (int is = 0; is != oPointSecIdxs.size(); ++is) {
        for (int j = 0; j != oPointSecIdxs[is].size(); ++j) {
            
            int iPointIdx = oPointSecIdxs[is][j];
            cloud_label.points[iPointIdx].x = process_Cloud.points[iPointIdx].x;
            cloud_label.points[iPointIdx].y = process_Cloud.points[iPointIdx].y;
            cloud_label.points[iPointIdx].z = process_Cloud.points[iPointIdx].z;
            if (vAllGroundRes[is][j] == 1) {
                cloud_label.points[iPointIdx].label = 28;
            }else {
                cloud_label.points[iPointIdx].label = 0;
            }
        }//end for j
    }//end for i
