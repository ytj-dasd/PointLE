...
TEST_DATASET = modelnet_h5_dataset.ModelNetH5Dataset(os.path.join(BASE_DIR, 'data/modelnet40_ply_hdf5_2048/vehicle_test_files.txt'), batch_size=BATCH_SIZE, npoints=1024,shuffle=False)
...
def test(num_votes):
    ...
    test_one_epoch(sess, ops, num_votes)

def test_one_epoch(sess, ops, num_votes=1, topk=1):
    is_training = False
    #is_training = True
    ...
    while TEST_DATASET.has_next_batch():
        ...
        print('score: ',float(object_score[0,9]))
...
if __name__=='__main__':
    with tf.Graph().as_default():
        test(num_votes=FLAGS.num_votes)
    LOG_FOUT.close()
